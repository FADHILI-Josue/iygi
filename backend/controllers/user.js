...
export const registerNewUser = asyncHandler(async (req, res) => {
  ...
  if (checkIfEmailIsTaken) {
    res.status(400).json("email is arleady in use");
    throw new Error("Email is already in use.");
  }

  if (checkIfUserNameIsTaken) {
    res.status(400).json("username have been taken");
...
export const followUser = asyncHandler(async (req, res, next) => {
  const user = await User.findOne({ userName: req.params.username }).select(
    "_id followers following followerCount followingCount"
  );
...
export const getUserbyUsername = asyncHandler(async (req, res, next) => {
  const user = await User.findOne({ userName: req.params.username })
    .select(
      "_id website userName fullName profilePic followers following followerCount followingCount postCount bio posts"
    )
    .populate("posts following followers");
  const userFollowerIds = await User.findOne({
